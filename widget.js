...
window.addEventListener('load', () => {
    for (let widget of document.getElementsByTagName('discord-widget')) {
        ...
        //style
        widget.innerHTML = '<link rel="stylesheet" href="https://affonsogames.github.io/plugins/widget.js">';
        ...
        fetch(`https://discord.com/api/guilds/${id}/widget.json`).then(data => {
            data.json().then(data => {
                //member count
                count.innerHTML = `<center><strong>${data.presence_count - 1}</strong> Jogadores online`</center>;
                //join button
